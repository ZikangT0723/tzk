...
//USER FUNCTION 
bool userLogin(int* index); // Pavan
...
int main() {
	...
	LoadTest1();
	int choice, index =0; //added index = 0 
	...
	while (true)
	{
		...
		case 2:
			...
			//control structure to pick between login or register 
			/*
	...
	if (option == 1) {
		...
		if (userindex < Studentnum) {
			*index = getinfo(Student);
			...
			cout << "The userlist is full." << endl;
			return false; // for now return -1 havent debugged yet
		}
	}
}
